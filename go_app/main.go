...
var httpRequestsTotalContact = prometheus.NewCounterVec(prometheus.CounterOpts{
	Name: "goapp_http_requests_contact_total",
	Help: "Count of all HTTP requests for goapp",
}, []string{})
...
func main() {
	...
	r.MustRegister(httpRequestsTotalContact)
	...
	c := promhttp.InstrumentHandlerDuration(
		httpDuration.MustCurryWith(prometheus.Labels{"handler": "contact"}),
		promhttp.InstrumentHandlerCounter(httpRequestsTotalContact, contact),
	)
